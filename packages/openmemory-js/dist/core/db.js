...
if (is_pg) {
    ...
    const init = async () => {
        ...
        await pg.query(`create table if not exists ${m}(
                tenant_id text default '${cfg_1.env.default_tenant_id}',
                id uuid,
                user_id text,
                ...
                primary key(${cfg_1.env.multi_tenant ? 'tenant_id, id' : 'id'})
            )`);
        // Create vectors table with optional pgvector support
        const vectorType = cfg_1.env.pgvector_enabled ? `vector(${cfg_1.env.vec_dim})` : 'bytea';
        await pg.query(`create table if not exists ${v}(
                tenant_id text default '${cfg_1.env.default_tenant_id}',
                id uuid,
                sector text,
                user_id text,
                ...
                primary key(${cfg_1.env.multi_tenant ? 'tenant_id, id, sector' : 'id, sector'})
            )`);
        await pg.query(`create table if not exists ${w}(
                tenant_id text default '${cfg_1.env.default_tenant_id}',
                src_id text,
                ...
                created_at bigint,
                updated_at bigint,
                primary key(${cfg_1.env.multi_tenant ? 'tenant_id, src_id, dst_id' : 'src_id, user_id'})
            )`);
        await pg.query(`create table if not exists ${l}(
                id text primary key,
                tenant_id text default '${cfg_1.env.default_tenant_id}',
                model text,
                status text,
        ...
        await pg.query(`create table if not exists "${sc}"."openmemory_users"(
                tenant_id text default '${cfg_1.env.default_tenant_id}',
                user_id text,
                summary text,
                ...
                created_at bigint,
                updated_at bigint,
                primary key(${cfg_1.env.multi_tenant ? 'tenant_id, user_id' : 'user_id'})
            )`);
        await pg.query(`create table if not exists "${sc}"."stats"(
        ...
        if (cfg_1.env.multi_tenant) {
            await pg.query(`create index if not exists openmemory_memories_tenant_sector_idx on ${m}(tenant_id, primary_sector)`);
            await pg.query(`create index if not exists openmemory_memories_tenant_segment_idx on ${m}(tenant_id, segment)`);
            await pg.query(`create index if not exists openmemory_memories_tenant_simhash_idx on ${m}(tenant_id, simhash)`);
            await pg.query(`create index if not exists openmemory_memories_tenant_user_idx on ${m}(tenant_id, user_id)`);
            await pg.query(`create index if not exists openmemory_vectors_tenant_user_idx on ${v}(tenant_id, user_id)`);
            await pg.query(`create index if not exists openmemory_vectors_tenant_sector_idx on ${v}(tenant_id, sector)`);
            await pg.query(`create index if not exists openmemory_waypoints_tenant_src_idx on ${w}(tenant_id, src_id)`);
            await pg.query(`create index if not exists openmemory_waypoints_tenant_dst_idx on ${w}(tenant_id, dst_id)`);
            await pg.query(`create index if not exists openmemory_stats_tenant_ts_idx on "${sc}"."stats"(tenant_id, ts)`);
            await pg.query(`create index if not exists openmemory_stats_tenant_type_idx on "${sc}"."stats"(tenant_id, type)`);
        }
        else {
        ...
        if (cfg_1.env.vector_backend === "valkey") {
            ...
            exports.vector_store = vector_store = new postgres_1.PostgresVectorStore({ run_async, get_async, all_async }, v.replace(/"/g, ""), cfg_1.env.pgvector_enabled // Pass pgvector flag
            );
            const mode = cfg_1.env.pgvector_enabled ? 'pgvector (native)' : 'BYTEA (in-memory)';
    ...
    db.serialize(() => {
        ...
        db.run(`create table if not exists ${sqlite_vector_table}(
                id text not null,
        ...
        db.run(`create table if not exists embed_logs(
                id text primary key,
                tenant_id text default '${cfg_1.env.default_tenant_id}',
                model text,
                status text,
    ...
    if (cfg_1.env.vector_backend === "valkey") {
        ...
        exports.vector_store = vector_store = new postgres_1.PostgresVectorStore({ run_async, get_async, all_async }, sqlite_vector_table, false // pgvector not supported in SQLite
        );
    ...
    exports.q = q = {
        ...
        ins_user: {
            run: (...p) => exec("insert or ignore into users(user_id,summary,reflection_count,created_at,updated_at) values(?,?,?,?,?)", p),
        },
        get_user: {
